...
\frac{\partial^2 l_{ij}}{\partial p_{ij}^2} &= \sum\limits_{i=1}^{n_j}\left\{ -\delta_{ij} \left(\frac{\exp(-\lambda_{ij}) - 1}{(1-p_{ij}) + p_{ij} \exp(-\lambda_{ij})}\right)^2 - (1-\delta_{ij}) \frac{1}{p_{ij}^2} \right\}\\
\frac{\partial^2 l_{ij}}{\partial p_{ij} \partial \lambda_{ij}} &=  \delta_{ij} \left(\frac{-(\exp(-\lambda_{ij}) - 1)\exp(-\lambda_{ij})}{(1-p_{ij}) + p_{ij} \exp(-\lambda_{ij})} - \frac{(\exp(-\lambda_{ij}) - 1)p_{ij}\exp(-\lambda_{ij})}{((1-p_{ij}) + p_{ij} \exp(-\lambda_{ij}))^2} \right)\\
\frac{\partial^2 l_j}{\partial \lambda_{ij}^2} &=  \delta_{ij} \left[\frac{p_{ij}\exp(-\lambda_{ij})}{(1-p_{ij}) + p_{ij} \exp(-\lambda_{ij})} -\left(\frac{p_{ij}\exp(-\lambda_{ij})}{(1-p_{ij}) + p_{ij} \exp(-\lambda_{ij})}\right)^2\right] - (1-\delta_{ij}) \left[\frac{y_{ij}}{\lambda_{ij}^2}\right] \\
