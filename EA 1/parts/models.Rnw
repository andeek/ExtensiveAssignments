In this section we detail the model formulation for each of our three models. The setup of random variables, as well as their distributions are discussed below. For detetails fo these models, including first and second derivatives used to fit the models, see the appendices.
