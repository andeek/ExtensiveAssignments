...
&= \log\{f_2(y_i|\boldsymbol \theta_2)\} + [\log\{f_1(y_i|\boldsymbol \theta_1)\} - \log\{f_2(y_i|\boldsymbol \theta_2)\}]\text{E}_{z|y}(z_i  | \boldsymbol \theta_p \phi_p ) \\
&= \alpha_2 \log \beta_2 - \log(\Gamma(\alpha_2)) + (\alpha_2 - 1) \log y_i - \beta_2 y_i + \\
& \qquad [\alpha_1 \log \beta_1 - \log(\Gamma(\alpha_1)) + (\alpha_1 - 1) \log y_i - \beta_1 y_i - \\
& \qquad \qquad \alpha_2 \log \beta_2 + \log(\Gamma(\alpha_2)) + (\alpha_2 - 1) \log y_i + \beta_2 y_i]\text{E}_{z|y}(z_i  | \boldsymbol \theta_p \phi_p )
\end{align*}
...
&= \log(1-\phi) + \log\left(\frac{\phi}{1-\phi}\right)\text{E}_{z|y}[z_i|\boldsymbol \theta_p, \phi_p]
\end{align*}
...
\begin{align*}
\text{E}_{z|y}[z_i|\boldsymbol \theta_p, \phi_p] &= \sum\limits_{z_i \in \Omega_z} \left[ \frac{\left(\phi_p \frac{\beta_{p1}^{\alpha_{p1}}}{\Gamma(\alpha_{p1})}y_i^{\alpha_{p1} - 1}e^{-\beta_{p1} y_i}\right)^{z_{i}}\left((1-\phi_p) \frac{\beta_{p2}^{\alpha_{p2} }}{\Gamma(\alpha_{p2})}y_i^{\alpha_{p2} - 1}e^{-\beta_{p2} y_i}\right)^{(1-z_{i})}}{\phi_p \frac{\beta_{p1}^{\alpha_{p1} }}{\Gamma(\alpha_{p1})}y_i^{\alpha_{p1} - 1}e^{-\beta_{p1} y_i} + (1-\phi_p) \frac{\beta_{p2}^{\alpha_{p2} }}{\Gamma(\alpha_{p2})}y_i^{\alpha_{p2} - 1}e^{-\beta_{p2} y_i}} z_i \right] \\
&= \left[ \frac{\left(\phi_p \frac{\beta_{p1}^{\alpha_{p1}}}{\Gamma(\alpha_{p1})}y_i^{\alpha_{p1} - 1}e^{-\beta_{p1} y_i}\right)}{\phi_p \frac{\beta_{p1}^{\alpha_{p1} }}{\Gamma(\alpha_{p1})}y_i^{\alpha_{p1} - 1}e^{-\beta_{p1} y_i} + (1-\phi_p) \frac{\beta_{p2}^{\alpha_{p2} }}{\Gamma(\alpha_{p2})}y_i^{\alpha_{p2} - 1}e^{-\beta_{p2} y_i}}\right]
\end{align*}
Substitution back into the expression for $Q$ yields:
\begin{align*}
Q(\boldsymbol \theta, \phi|\boldsymbol \theta_p, \phi_p) &= \sum\limits_{i=1}^n\Bigg\{\Bigg [ \alpha_2 \log \beta_2 - \log(\Gamma(\alpha_2)) + (\alpha_2 - 1) \log y_i - \beta_2 y_i + \\
& \qquad [\alpha_1 \log \beta_1 - \log(\Gamma(\alpha_1)) + (\alpha_1 - 1) \log y_i - \beta_1 y_i - \alpha_2 \log \beta_2 + \log(\Gamma(\alpha_2)) + (\alpha_2 - 1) \log y_i + \beta_2 y_i] +\\
& \qquad \log(1-\phi) + \log\left(\frac{\phi}{1-\phi}\right)\Bigg ] \left[ \frac{\left(\phi_p \frac{\beta_{p1}^{\alpha_{p1}}}{\Gamma(\alpha_{p1})}y_i^{\alpha_{p1} - 1}e^{-\beta_{p1} y_i}\right)}{\phi_p \frac{\beta_{p1}^{\alpha_{p1} }}{\Gamma(\alpha_{p1})}y_i^{\alpha_{p1} - 1}e^{-\beta_{p1} y_i} + (1-\phi_p) \frac{\beta_{p2}^{\alpha_{p2} }}{\Gamma(\alpha_{p2})}y_i^{\alpha_{p2} - 1}e^{-\beta_{p2} y_i}}\right]\Bigg\}
\end{align*}

Computation of this expression is the E-step of one iteration in our EM algorith. The M-step is the maimization of this expression for $Q$ in $\boldsymbol \theta$ and $\phi$.
