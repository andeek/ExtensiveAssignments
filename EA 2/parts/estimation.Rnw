...
\end{align*}
Thus, we can write the functons $L$, $Q$, and $H$ n our formulation of the EM algorthm as sums over $i$. Let $\boldsymbol \theta = (\alpha_1,\beta_1,\alpha_2,\beta_2)$, then
\begin{align*}
...
Computation of this expression is the E-step of one iteration in our EM algorithm. The M-step is the maximization of this expression of $Q$ in $\boldsymbol \theta$ and $\phi$. We will use a Newton-Raphson algorithm to maximize $Q = \sum Q_i$. The first and second derivatives necessary for this step are below.
\begin{align*}
