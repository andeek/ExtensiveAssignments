...
\begin{align*}
\frac{\partial f_j(y_i|\gamma_i,\lambda_j)}{\partial \lambda_j} &= \frac{e^{-\lambda_j\gamma_i}\gamma_i^{y_i}\lambda_j^{y_i-1}}{y_i!}(\lambda_j - \gamma_i) \\
 \frac{\partial^2 f_j(y_i|\gamma_i,\lambda_j)}{\partial \lambda_j^2} &= \frac{e^{-\lambda_j\gamma_i}\gamma_i^{y_i}\lambda_j^{y_i-2}}{y_i!}(\gamma_i + y_i(\lambda_j -\gamma_i) - \gamma_i\lambda_i(\lambda_j - \gamma_i))
\end{align*}
...
Applying these procedures yields the following estimated using the convergence criterion of $\delta = 10^{-8}$.
