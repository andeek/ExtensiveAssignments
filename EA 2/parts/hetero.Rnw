...
print(xtable(res, caption="Maximum likelihood estimaes and 95\\% Wald intervals for parameters of the two component poisson distributions.", label="tab:hetero"), sanitize.text.function=function(x){x})
inf_print <- paste(apply(solve(h),1,function(x){paste(paste(round(x,4), collapse="&"),"\\\\",sep="")}),collapse=" ")
@
With starting values of $\phi^{(0)} = .5, \lambda_1^{(0)} = 0.01, \lambda_2^{(0)} = 5$, the algorithm converged in \Sexpr{ests['n.iter']} iterations. The inverse observed information matrix used to compute the interval estimates of Table~\ref{tab:hetero} was, 
\begin{align*}
\mathcal{I}^{-1} = 
\left(\begin{matrix} 
\Sexpr{inf_print}
...
@
Using the estimates from Table~\ref{tab:hetero}, expected flaws were computed for each segment of track. These are reporten in Table~\ref{tab:hetero2}, along with individual contributions to the overall Chi-square test statistic of \Sexpr{chi.test} with, which with \Sexpr{nrow(subway.dat) - 1} degress of freedom, has an associated $p$-value of \Sexpr{1-pchisq(chi.test, nrow(subway.dat)-1)}. This $p$-value indicates a substantial improvement in the heterogeneous model for flaws in subway track segments. 
{\footnotesize
