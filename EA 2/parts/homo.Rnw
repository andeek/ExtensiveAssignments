We define random variables $\{Y_i: i = 1, \dots, n\}$ to be connected with the number of flaws on track section $i$. In our case, $n = $\Sexpr{length(unique(subway.dat$segment))}. As an initial model, we consider a constant rate of flaws $\lambda$ for all track sections, $i$. $Y_i$ are assumed to be independent with poisson probability mass functions for $\lambda > 0$,
\begin{align*}
...
@
To assess the model fit, we use the Pearson Chi-square test which gives a test statistics value of \Sexpr{round(chi.test,4)}, which with \Sexpr{nrow(subway.dat) - 1} degrees of freedom yields a $p$-value of nearly \Sexpr{1-pchisq(chi.test, nrow(subway.dat)-1)}. The Chi-square test indicates that the constant poisson model cannot adequately represent number of flaws in a subway tracks. We can investigate whether there are particular sections of track that fit poorly by looking at the individual contributions to the Chi-square test statistic. See Table~\ref{tab:homo} for details.
{\footnotesize
...
The highest contributions come from segments with zero and one flaw, as well as contributions from segments with 15 and 18 flaws. These segments account for \Sexpr{round(sum(subway.dat[subway.dat$flaws %in% c(0,1,15,18),"chi_sq_homo"])/sum(subway.dat$chi_sq_homo)*100,2)}\% of the total $\chi^2$ test statistic value. 
