...
The highest contributions come from segments with zero and one flaw, as well as contributions from segments with 15 and 18 flaws. These segments account for \Sexpr{perc.chi}\% of the total $\chi^2$ test statistic value. %We then hypothesize that these two groups may correspond to flaws from type A and type B steel tracks, respectively.  
