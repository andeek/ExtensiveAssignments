...
subway.dat$chi_sq_homo <- with(subway.dat, (flaws - exp.counts)^2/exp.counts)
chi.test_homo<-sum(subway.dat$chi_sq_homo)
@
As a quick check we can compute a Pearson Chi-square test which gives a test statistics value of \Sexpr{round(chi.test_homo,4)}, which with \Sexpr{nrow(subway.dat) - 1} degrees of freedon yields a $p$-value of nearly \Sexpr{1-pchisq(chi.test_homo, nrow(subway.dat)-1)}. We can investigate where there are particular sections of track that fit poorly by looking at the individual contributions to the Chi-square test statistic. See Table~\ref{tab:homo} for details.
{\footnotesize
