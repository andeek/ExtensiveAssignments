...
<<data_plot, echo=FALSE, fig.height=4, fig.width=6, out.width='.6\\textwidth', fig.cap='Number of flaws per km of track for each track segment.'>>=
qplot(x=segment, ymin=0, ymax=rate, data=subway.dat, geom="linerange") +
  ylab("Flaws per km") + xlab("Track Segment")
@

<<data_summary, echo=FALSE>>=
summary(subway.dat)
@
